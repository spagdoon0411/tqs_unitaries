...
class TQSPositionalEncoding1D(nn.Module):
    ...
    def __init__(self, d_model, param_dim, max_len=500, dropout=0):
        ...
        position = torch.arange(0, max_len, dtype=torch.get_default_dtype()).unsqueeze(
            1
        )
        div_term = torch.exp(
            torch.arange(0, d_model, 2, dtype=torch.get_default_dtype())
        ...
        self.pe = torch.cat(
            [self.param_embedding, pe], dim=0
        )  # (param_dim+n, 1, d_model)
...
class TQSPositionalEncoding2D(nn.Module):
    ...
    def __init__(self, d_model, param_dim, max_system_size=None, dropout=0):
        ...
        div_term = torch.exp(
            torch.arange(0, channels, 2, dtype=torch.get_default_dtype())
