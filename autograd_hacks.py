...
_enforce_fresh_backprop: bool = (
    False  # global switch to catch double backprop errors on Hessian computation
)
...
def remove_hooks(model: nn.Module) -> None:
    ...
    assert model == 0, (
        "not working, remove this after fix to https://github.com/pytorch/pytorch/issues/25723"
    )
...
def compute_grad1(model: nn.Module, loss_type: str = "mean", grad_name="grad1") -> None:
    """
    ...
    for layer in model.modules():
        ...
        A = layer.activations
        A_shape = A.shape
        A1 = A.reshape(-1, A_shape[-1])
        n = A1.shape[0]

        if not hasattr(layer, "backprops_list"):
            setattr(layer.weight, grad_name, torch.zeros((n,) + layer.weight.shape))
            setattr(layer.bias, grad_name, torch.zeros((n,) + layer.bias.shape))
            # print(f"Warning, no backprops detected for {layer}, setting {grad_name} to zero")
            continue
        else:
            ...
            B_shape = B.shape
            B1 = B.reshape(-1, B_shape[-1])

            if layer_type == "Linear":
                # grad_shape = B_shape + A_shape[-1:]
                grad1 = torch.einsum("ni,nj->nij", B1, A1)
                # grad1 = grad1.view(grad_shape)
                # assert torch.allclose(grad1.sum(dim=0), layer.weight.grad, rtol=1e-3, atol=1e-3)
                setattr(layer.weight, grad_name, grad1)
                if layer.bias is not None:
                    setattr(layer.bias, grad_name, B1)
                    # assert torch.allclose(B1.sum(dim=0), layer.bias.grad, rtol=1e-3, atol=1e-3)
                ...
                setattr(layer.weight, grad_name, grad1.reshape(shape))
                if layer.bias is not None:
                    setattr(layer.bias, grad_name, torch.sum(B, dim=2))
...
def compute_hess(
    ...
    for layer in model.modules():
        ...
        if layer_type == "Linear":
            ...
            n = A.shape[0]
            B = torch.stack(
                [Bt.reshape(n, do, -1) for Bt in layer.backprops_list]
            )  # o, n, do, Oh*Ow
            o = B.shape[0]
            ...
            Hi = torch.einsum(
                "onij,onkl->nijkl", Jb, Jb
            )  # n, do, di*Kh*Kw, do, di*Kh*Kw
